...
const querySchema = z.object({
  ...
  titles: z.string().max(60).optional(),
...
export async function listingRoutes(app: FastifyInstance): Promise<void> {
  app.get('/api/listings', async (req, reply) => {
    ...
    const q: ListingQuery = {
      ...
      titles: csv(p.titles).filter((t): t is ListingQuery['titles'] extends Array<infer T> | undefined ? T : never => t === 'clean' || t === 'salvage' || t === 'rebuilt' || t === 'other'),
