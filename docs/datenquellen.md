...
| **VAE** | **Dubizzle** | Algolia-**Proxy** der Website: `POST https://algolia.dubizzle.com/1/indexes/*/queries`, Body `{"requests":[{"indexName":"motors.com","query":"","params":"page=0&hitsPerPage=1000&attributesToRetrieve=[…]&filters=(\"category_v2.slug_paths\":\"motors/used-cars\")"}]}`; App-ID und Search-Key setzt der Proxy serverseitig (Header `x-algolia-application-id`/`x-algolia-api-key` leer). Treffer: `uuid, name{en}, price (AED), absolute_url{en}, photos, details{Make, Model, Trim, Year, Kilometers, Fuel Type, Transmission Type, Regional Specs, Horsepower, No. of Cylinders, Steering Side, Engine Capacity (cc)}`, `details_v2{primary…: [{label, value, slug}]}`, `photo_thumbnails[]`, `location_list{en: [UAE, Emirat, Bezirk]}` **[B]** (Live-Probe 16.09.2026: 41.253 Gebrauchtwagen, direkt aus Node) | `providers/dubizzle.ts` **bestätigt** – Preisfenster (AED) unter der Algolia-Grenze von 1.000 Treffern (13 Startfenster à 1.300–2.900 Treffer, werden halbiert), Vollabgleich. Leistung („300 - 399 HP“) und Hubraum („2000 - 2499 cc“) sind Bereichsangaben der Verkäufer: Hubraum als Bereichsmitte, Leistung nicht übernommen. Zusätzlich gesehen: `POST uae.dubizzle.com/svc/search/api/v1/searches/` (`{"search_state":{"city":4,"category":["motors/used-cars"],"page":2}}`, braucht anonymes `x-access-token`-JWT) |
| VAE | Dubicars, YallaMotor | HTML | offen |
| **Japan** | **Jap Carz** (jap-carz.com) | JSON-API der Website: `GET https://jap-carz.com/api/listings/?sort=upcoming_auctions&per_page=30&page=1` → `{ data[], meta{current_page,last_page,per_page,total} }`; Item: `lot, slug, title, make{name}, model{name}, year, parsed_mileage ("288000 km"), parsed_displacement, parsed_transmission (AT/MT), parsed_starting_bid (JPY), parsed_final_price, steering_wheel (lhd/rhd), auction_date, auction_time (Japan), auction_result ("non Auction" = ausstehend), site, grade_label, cover, preview_images[]` **[B]** (Live-Probe 20.09.2026 ohne Cookie: HTTP 200, 475 Fahrzeuge, 16 Seiten) | `providers/japcarz.ts` **bestätigt** – Vollabgleich über alle Seiten; Linkslenker mit Startgebot und künftigem Termin; Detailpfad `JAPCARZ_DETAIL_PATH` (Standard `/listings/{slug}`, prüfen) |
